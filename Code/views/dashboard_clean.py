    ...
    if rf_model is not None and X_test is not None and y_test is not None:
        ...
        if st.button("Show Model Metrics", key="model_metrics"):
            ...
            with tab7:
                ...
                # Generate Complete Test Dataset CSV
                st.subheader("📊 Generate Complete Test Dataset CSV")
                
                st.info("💡 This generates a CSV containing ALL test records (20% of original dataset) with actual labels, predicted labels, and complete feature information.")
                ...
                if st.button("🗃️ Generate Complete Test Dataset CSV", key="generate_complete_test_csv"):
                    with st.spinner("Generating complete test dataset with all records..."):
                        try:
